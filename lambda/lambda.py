...
def lambda_handler(event, context):
    if fhir_data_path == PATH_NOT_SET:
        ...
        con = create_local_db_con()
    logger.info("Created connection")
    resource, fields, patients, offset, limit = extract_params(event)
    resources = get_fhir_resource_types()
    logger.info(f"Found the following resources: {resources}")
    if resource in resources:
        data = get_fhir_data(con, resource, fields, patients, offset, limit)
        count = get_fhir_count(con, resource, patients)
        con.close()
